...
# Station data disabled for now — commenting out imports that fetch station readings
# from water_temp import fetch_all_station_data, get_station_water_temp_for_hff
from weather_forecast import get_forecast, get_current_conditions, PREDEFINED_LOCATIONS
...
# Page configuration
st.set_page_config(
    page_title="Philadelphia Ice Forecasting",
    page_icon="🧊",
    layout="wide"
)
...
# Main app
st.title("Philadelphia Ice Forecasting Tool")
...
location_option = st.radio(
    "Select a location:",
    options=['Philadelphia, PA - Baxter Water Intake', "Philadelphia, PA - Schuylkill Rv. Near 30th St", 
             "Trenton, NJ - Calhoun St Bridge", "Custom Location"],
    horizontal=False
)

# Initialize session state for custom location
if 'custom_lat' not in st.session_state:
    st.session_state.custom_lat = 39.9526
if 'custom_lon' not in st.session_state:
    st.session_state.custom_lon = -75.1652
if 'custom_elev' not in st.session_state:
    st.session_state.custom_elev = 50

# Get selected location parameters
if location_option == "Custom Location":
    col1, col2, col3 = st.columns(3)
    with col1:
        st.session_state.custom_lat = st.number_input(
            "Latitude",
            value=st.session_state.custom_lat,
            format="%.6f",
            step=0.000001,
            key='custom_lat_input'
        )
    with col2:
        st.session_state.custom_lon = st.number_input(
            "Longitude",
            value=st.session_state.custom_lon,
            format="%.6f",
            step=0.000001,
            key='custom_lon_input'
        )
    with col3:
        st.session_state.custom_elev = st.number_input(
            "Elevation (ft)",
            value=st.session_state.custom_elev,
            step=1,
            key='custom_elev_input'
        )
    selected_location = {
        'lat': st.session_state.custom_lat,
        'lon': st.session_state.custom_lon,
        'elev': st.session_state.custom_elev
    }
    display_location_name = f"Custom ({selected_location['lat']:.6f}, {selected_location['lon']:.6f})"
else:
    selected_location = PREDEFINED_LOCATIONS[location_option]
    display_location_name = location_option

st.info(f"**Active Location:** {display_location_name} (Elev: {selected_location['elev']} ft)")

st.divider()

# Station data fetching disabled — skip loading station readings
...
forecast_data = load_forecast(selected_location['lat'], selected_location['lon'])

# Create DataFrame
# Station data disabled — use empty DataFrame
...
# # Station-derived temperature fallback disabled — force default
# station_temp_c = None
# station_source = 'Default'
...
# else:
#     default_water_temp = 0.2
#     water_temp_source_label = 'Default'

with col1:
    hf_lat = st.number_input('Heat Flux Latitude', value=float(selected_location.get('lat', 41.1242)), format="%.6f", step=0.000001, key='hf_lat')
with col2:
    hf_lon = st.number_input('Heat Flux Longitude', value=float(selected_location.get('lon', -101.3644337)), format="%.6f", step=0.000001, key='hf_lon')
with col3:
    T_water_C = st.number_input('Water Temperature (°C)', value=0.2, key='T_water')
with col4:
    D = st.number_input('Characteristic Depth (m)', value=2.0, key='depth')

# Display active location info
st.markdown(f"**Active Location:** {display_location_name} — {hf_lat:.6f}, {hf_lon:.6f}")
...
if st.button('🌡️ Compute Heat Fluxes'):
    try:
        # Use selected location for heat flux calculations
